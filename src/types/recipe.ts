...
export function isValidCharacteristicType(
  ...
  return CHARACTERISTIC_TYPES.includes(
    characteristicType as CharacteristicType
  );
}
...
export function validateRecipeInput(input: RecipeInput): {
  ...
  if (input.diet_types) {
    const invalidDietTypes = input.diet_types.filter(
      (dietType) => !isValidDietType(dietType)
    );
    if (invalidDietTypes.length > 0) {
      errors.push(
        `Invalid diet types: ${invalidDietTypes.join(
          ", "
  ...
  if (input.cooking_methods) {
    const invalidCookingMethods = input.cooking_methods.filter(
      (method) => !isValidCookingMethod(method)
    );
    if (invalidCookingMethods.length > 0) {
      errors.push(
        `Invalid cooking methods: ${invalidCookingMethods.join(
          ", "
  ...
  if (input.dish_types) {
    const invalidDishTypes = input.dish_types.filter(
      (dishType) => !isValidDishType(dishType)
    );
    if (invalidDishTypes.length > 0) {
      errors.push(
        `Invalid dish types: ${invalidDishTypes.join(
          ", "
  ...
  if (input.proteins) {
    const invalidProteins = input.proteins.filter(
      (protein) => !isValidProteinType(protein)
    );
    if (invalidProteins.length > 0) {
      errors.push(
        `Invalid proteins: ${invalidProteins.join(
          ", "
  ...
  if (input.occasions) {
    const invalidOccasions = input.occasions.filter(
      (occasion) => !isValidOccasionType(occasion)
    );
    if (invalidOccasions.length > 0) {
      errors.push(
        `Invalid occasions: ${invalidOccasions.join(
          ", "
  ...
  if (input.characteristics) {
    const invalidCharacteristics = input.characteristics.filter(
      (characteristic) => !isValidCharacteristicType(characteristic)
    );
    if (invalidCharacteristics.length > 0) {
      errors.push(
        `Invalid characteristics: ${invalidCharacteristics.join(
          ", "
