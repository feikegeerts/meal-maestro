...
export const handlers = [
  ...
  // User profiles endpoints - handle both single and list responses
  http.get(`${SUPABASE_URL}/rest/v1/user_profiles`, ({ request }) => {
    ...
    // Handle single record request (.single() calls)
    if (id === "eq.test-user-id") {
      ...
      if (preferHeader && preferHeader.includes("return=representation")) {
        return HttpResponse.json(mockProfile); // Single object for .single() calls
      }

      return HttpResponse.json([mockProfile]); // Array for normal queries
    }

    if (id === "eq.non-existent-user") {
      return HttpResponse.json([]);
    }
  ...
  http.patch(`${SUPABASE_URL}/rest/v1/user_profiles`, async ({ request }) => {
    ...
    // Check if single response expected
    const preferHeader = request.headers.get("Prefer");
    if (preferHeader && preferHeader.includes("return=representation")) {
      return HttpResponse.json(updatedProfile); // Single object for .single() calls
    }
