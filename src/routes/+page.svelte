<script lang="ts">
  ...
  // Use a regular fetch with a relative path to the static directory
  onMount(() => {
    fetch('/data/careerEvents.json')
      .then((response) => {
        if (!response.ok) {
          throw new Error('Failed to fetch career events data: ' + response.status);
        }
