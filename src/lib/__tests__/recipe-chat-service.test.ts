...
process.env.RESEND_FROM_EMAIL = 'Meal Maestro <noreply@example.com>';
process.env.RESEND_REPLY_TO = 'support@example.com';
process.env.NEXT_PUBLIC_SUPABASE_URL = 'https://example.supabase.co';
process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY = 'public-anon-key';
process.env.SUPABASE_WEBHOOK_SECRET = 'secure-webhook-secret-with-sufficient-length';
...
interface SupabaseStubOptions {
  profile?: { unit_system_preference?: string } | null;
  profileError?: Error | null;
  units?: Array<{ unit_name: string | null }> | null;
  unitsError?: Error | null;
  disableCustomUnits?: boolean;
}

const createSupabaseStub = (options: SupabaseStubOptions = {}) => {
  const profileResult = options.profile ?? null;
  ...
  const profileQuery = (() => {
    const builder: {
      ...
      maybeSingle: jest.Mock<
        Promise<{ data: { unit_system_preference?: string } | null; error: null }>,
        []
      >;
    } = {
      select: jest.fn(),
  ...
  let unitsQuery: {
    select: jest.Mock;
    eq: jest.Mock;
    order: jest.Mock<Promise<{ data: Array<{ unit_name: string | null }> | null; error: null }>, [string, { ascending?: boolean }?]>;
  } | null = null;

  if (!options.disableCustomUnits) {
    const unitsData = options.units ?? [];
    const unitsError = options.unitsError ?? null;
    ...
    unitsQuery.select.mockReturnValue(unitsQuery);
    unitsQuery.eq.mockReturnValue(unitsQuery);
    unitsQuery.order.mockImplementation(async () => {
      if (unitsError) {
        throw unitsError;
      }
  ...
  const fromMock = jest.fn((table: string) => {
    if (table === 'user_profiles') {
      return profileQuery as unknown;
    }
    if (table === 'custom_units') {
      if (!unitsQuery) {
        throw new Error('Custom units query should not be invoked');
      }
      return unitsQuery as unknown;
    }
  ...
  return { supabase, fromMock, profileQuery, unitsQuery };
};

jest.mock('@supabase/supabase-js', () => {
  interface SelectChain {
    select: jest.Mock<SelectChain, [string?, unknown?]>;
    eq: jest.Mock<SelectChain, [string?, unknown?]>;
    order: jest.Mock<SelectChain, [string?, { ascending?: boolean }?]>;
    maybeSingle: jest.Mock<Promise<{ data: null; error: null }>, []>;
  }

  const createSelectChain = (): SelectChain => {
    const chain: Partial<SelectChain> = {};

    chain.select = jest.fn(() => chain as SelectChain);
    chain.eq = jest.fn(() => chain as SelectChain);
    chain.order = jest.fn(() => chain as SelectChain);
...
describe('RecipeChatService', () => {
  const ConversationBuilderMock = ConversationBuilder as unknown as jest.MockedClass<typeof ConversationBuilder>;
  const FunctionCallProcessorMock = FunctionCallProcessor as unknown as jest.MockedClass<typeof FunctionCallProcessor>;
  const ChatResponseFormatterMock = ChatResponseFormatter as unknown as jest.MockedClass<typeof ChatResponseFormatter>;
  const createChatCompletionTypedMock = createChatCompletion as jest.MockedFunction<typeof createChatCompletion>;
  const logUsageTypedMock = usageTrackingService.logUsage as jest.MockedFunction<typeof usageTrackingService.logUsage>;
  const assertWithinMonthlyLimitTypedMock = usageLimitService.assertWithinMonthlyLimit as jest.MockedFunction<typeof usageLimitService.assertWithinMonthlyLimit>;
  ...
  beforeEach(() => {
    ...
    ConversationBuilderMock.mockImplementation(() => builderMocks as unknown as ConversationBuilder);

    processorMocks = {
      processFunctionCall: processFunctionCallMock,
    };
    FunctionCallProcessorMock.mockImplementation(() => processorMocks as unknown as FunctionCallProcessor);
    FunctionCallProcessorMock.getAvailableFunctions = getAvailableFunctionsMock;

    formatterMocks = {
      formatResponse: formatResponseMock,
    };
    ChatResponseFormatterMock.mockImplementation(() => formatterMocks as unknown as ChatResponseFormatter);

    createChatCompletionTypedMock.mockImplementation((...args) => createChatCompletionMock(...args));
    logUsageTypedMock.mockImplementation((...args) => logUsageMock(...args));
    assertWithinMonthlyLimitTypedMock.mockImplementation((...args) => assertWithinMonthlyLimitMock(...args));
  ...
  it('processes a standard chat message and returns formatted response', async () => {
    const chatMessages = [{ role: 'user', content: 'Hello' }] as ChatMessage[];
    const updatedHistory = [{ role: 'assistant', content: 'History' }] as ChatMessage[];
    ...
    expect(ConversationBuilder).toHaveBeenCalledWith('en', 'traditional-metric', []);
    ...
    expect(getAvailableFunctionsMock).toHaveBeenCalledWith('traditional-metric', []);
    ...
    expect(formatterMocks.formatResponse).toHaveBeenCalledWith('Assistant reply', updatedHistory, null);
  ...
  it('performs a follow-up completion when initial response only returns a function call', async () => {
    const chatMessages = [{ role: 'user', content: 'Scrape recipe' }] as ChatMessage[];
    const updatedHistory = [{ role: 'assistant', content: 'History' }] as ChatMessage[];
    ...
    processorMocks.processFunctionCall.mockResolvedValueOnce({
      functionResult: { function: 'update_recipe_form', result: { title: 'Filled' } },
    ...
    const formattedResponse: ChatResponse = {
      ...
      conversation_history: updatedHistory,
      function_call: { function: 'update_recipe_form', result: { title: 'Filled' } },
    };
    formatterMocks.formatResponse.mockResolvedValueOnce(formattedResponse);

    const service = new RecipeChatService('user-123');

    const result = await service.processMessage({ message: 'Scrape this recipe please' });

    expect(assertWithinMonthlyLimitMock).toHaveBeenCalledTimes(2);
    expect(createChatCompletionMock).toHaveBeenCalledTimes(2);
    expect(createChatCompletionMock).toHaveBeenNthCalledWith(2, expect.any(Array), [], 'none');
  ...
  it('appends follow-up answer for mixed URL requests', async () => {
    const chatMessages = [{ role: 'user', content: 'Check url' }] as ChatMessage[];
    const updatedHistory = [{ role: 'assistant', content: 'History' }] as ChatMessage[];
    ...
    processorMocks.processFunctionCall.mockResolvedValueOnce({
      functionResult: { function: 'extract_recipe_from_url', result: { title: 'Parsed' } },
    ...
    const formattedResponse: ChatResponse = {
      ...
      conversation_history: updatedHistory,
      function_call: { function: 'extract_recipe_from_url', result: { title: 'Parsed' } },
    };
    formatterMocks.formatResponse.mockResolvedValueOnce(formattedResponse);

    const service = new RecipeChatService('user-456');

    const message = 'https://example.com Can you also list the macros?';
  ...
  it('throws a MonthlySpendLimitError when usage tracking signals the limit is reached', async () => {
    ...
    await expect(service.processMessage({ message: 'Hello there' })).rejects.toBeInstanceOf(
  ...
  it('loads user preference and sanitized custom units from Supabase', async () => {
    RecipeChatService.clearCustomUnitsCache('user-pref');

    const { supabase, unitsQuery } = createSupabaseStub({
      profile: { unit_system_preference: 'imperial' },
      units: [
        { unit_name: ' Jar ' },
        { unit_name: 'Box' },
        { unit_name: 'Bad!' },
        { unit_name: null },
        { unit_name: 'way-too-long-custom-unit-name-for-test' },
      ],
    });

    builderMocks.buildMessages.mockReturnValue([{ role: 'user', content: 'Hello' }]);
    builderMocks.updateConversationHistory.mockReturnValue([{ role: 'assistant', content: 'Hi' }]);
    ...
    const service = new RecipeChatService('user-pref', 'nl', supabase);
    const result = await service.processMessage({ message: 'hello there' });

    expect(ConversationBuilder).toHaveBeenLastCalledWith('nl', 'imperial', ['Jar', 'Box']);
    ...
    expect(unitsQuery?.order).toHaveBeenCalledTimes(1);
    expect(result).toEqual(formattedResponse);
  });

  it('reuses cached custom units on subsequent instances', async () => {
    RecipeChatService.clearCustomUnitsCache('user-cache');

    const firstStub = createSupabaseStub({
    ...
    const firstService = new RecipeChatService('user-cache', 'en', firstStub.supabase);
    await firstService.processMessage({ message: 'first run' });

    const secondStub = createSupabaseStub({
      profile: null,
      disableCustomUnits: true,
    });

    builderMocks.buildMessages.mockClear();
    builderMocks.updateConversationHistory.mockClear();
    formatterMocks.formatResponse.mockClear();

    const secondService = new RecipeChatService('user-cache', 'en', secondStub.supabase);
    await secondService.processMessage({ message: 'second run' });

    const lastCall =
      ConversationBuilderMock.mock.calls[
        ConversationBuilderMock.mock.calls.length - 1
      ];
    expect(lastCall[2]).toEqual(['Cup']);
    expect(secondStub.fromMock).toHaveBeenCalledWith('user_profiles');
    expect(secondStub.fromMock).not.toHaveBeenCalledWith('custom_units');
  });
  ...
  it('continues with defaults when profile query fails', async () => {
    ...
    const service = new RecipeChatService('user-profile-fail', 'en', supabase);
  ...
  it('continues with empty custom units when unit query fails', async () => {
    ...
    const { supabase } = createSupabaseStub({
      profile: { unit_system_preference: 'mixed' },
      unitsError: new Error('units fail'),
    });
    ...
    const service = new RecipeChatService('user-units-fail', 'en', supabase);
  ...
  it('falls back to English messages when locale file is missing', () => {
    ...
    expect(translator.t('missing.translation.key')).toBe('missing.translation.key');
  });
  ...
  it('detectMixedRequest identifies URLs and additional content correctly', () => {
    ...
    expect(noUrl).toEqual({ hasUrl: false, hasAdditionalContent: false, additionalContent: '' });

    const onlyUrl = RecipeChatService.detectMixedRequest('https://example.com');
  ...
  it('logs error when second completion fails during two-call pattern', async () => {
    ...
    processorMocks.processFunctionCall.mockResolvedValueOnce({
      functionResult: { function: 'update_recipe_form', result: { title: 'Saved' } },
    ...
    formatterMocks.formatResponse.mockResolvedValueOnce({
      ...
      conversation_history: [{ role: 'assistant', content: 'Hi' }],
      function_call: { function: 'update_recipe_form', result: { title: 'Saved' } },
    });

    const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation(() => undefined);

    const service = new RecipeChatService('two-call-fail', 'en');
    const result = await service.processMessage({ message: 'trigger second call' });
